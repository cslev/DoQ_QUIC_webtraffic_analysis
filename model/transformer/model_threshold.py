...
for epoch in trange(N_EPOCHS, desc="Training"):
    ...
    for batch in tqdm(train_dataloader, desc=f"Epoch {epoch + 1} in training", leave=True, mininterval=50, miniters=50):
        ...
        if open_world:
            ...
            TP += ((predicted_labels == correct_labels) & (predicted_labels != unmonitored_class)).sum().item()
            FP += ((predicted_labels != unmonitored_class) & (correct_labels == unmonitored_class)).sum().item()
            TN += ((predicted_labels == correct_labels) & (correct_labels == unmonitored_class)).sum().item()
            FN += ((predicted_labels == unmonitored_class) & (correct_labels != unmonitored_class)).sum().item()
            WP += ((predicted_labels != correct_labels) & (predicted_labels != unmonitored_class) & (correct_labels != unmonitored_class)).sum().item()
    ...
    with torch.no_grad():
        ...
        test_loss = 0.0
        TP, FP, TN, FN, WP = 0, 0, 0, 0, 0
        for batch in tqdm(test_dataloader, desc=f"Epoch {epoch + 1} in testing", leave=True, mininterval=5, miniters=10):
            x, y = batch
            y = y.type(torch.LongTensor)
            x, y = x.to(device), y.to(device)
            y_hat = model(x)
        ...
        if open_world:
            print("TP, FP, TN, FN, WP: ")
            print(TP, FP, TN, FN, WP)
            print(f"Test TPR: {TP / (TP + WP + FN) * 100:.2f}%")
    ...
    # Test loop - threshold - every 10 epochs
    if epoch % 10 == 9 and open_world:
        threshold_range = [i * 0.02 for i in range(51)]
        threshold_list = []
        precision_list = []
        recall_list = []
        TP_list = []
        ...
        with torch.no_grad():
            for threshold in threshold_range:
                ...
                for batch in test_dataloader:
                    ...
                    predicted_labels = torch.argmax(predicted_probs, dim=1).detach().cpu()
                    correct_labels = y.detach().cpu()
                    
                    max_probs, _ = torch.max(predicted_probs, dim=1)
                    predicted_labels[max_probs < threshold] = unmonitored_class
                ...
                if TP + WP + FP != 0 and TP + WP + FN != 0:
                    precision = (TP + WP) / (TP + WP + FP)
                    recall = (TP + WP) / (TP + WP + FN)
...
filename = f"model_threshold_{current_time}.pth"
