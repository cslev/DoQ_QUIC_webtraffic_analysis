...
def packet_capture(url, index, output_path):
    ...
    subprocess.run("sudo cp /etc/resolv.conf .", shell=True)
    # Replace system dns configuration with DoQ proxy
    subprocess.run("sudo cp ./dns_config/resolv.conf /etc/resolv.conf", shell=True)
    trace_file_dir_path = ""
    try:
        domain = url.split("//")[1]
        ...
        process_tshark = multiprocessing.Process(target=trace_capture, args=(f"sudo tshark -w {trace_file} -f 'port 80 or port 443 or port 853'",))
        ...
        process_doq_proxy = multiprocessing.Process(target=doq_proxy, args=("sudo /users/KevinHZ/go/bin/dnsproxy -u quic://94.140.14.14/ -l 127.0.0.1",))
        ...
        subprocess.run("sudo pkill tshark", shell=True)
        subprocess.run("sudo python3 kill_doq.py", shell=True)
        time.sleep(1)
    except:
        subprocess.run(f"rm -rf {trace_file_dir_path}", shell=True)
    finally:
        # Restore system dns configuration
        subprocess.run("sudo mv ./resolv.conf /etc/", shell=True)
