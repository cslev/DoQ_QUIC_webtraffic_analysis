...
def parse_single_file(input_file_path, index_range: int = 200):
    ...
    count = 0
    sequence = []
    while count < min(index_range, len(df) - 1) and index < len(df) - 1:
        if int(protocols_onehot.iloc[index].values[0]):
            sequence.append([df['length'][index], df['relative_time'][index], directions[index]])
            count += 1
        index += 1
